...
Function GetModoFatu(aGetList,nLin,nCol,;      // menu em get
							lExib,nlinExib,nColExib,nLenExib)
	Local oGet := GetActive()
	Local aMatriz:={  {"N","VENDA          "},;
							{"E","ENTREGA FUTURA "},;
							{"S","SIMPLES REMESSA"} }
	Local nPos
	nPos:=ascan(aMatriz,{ |cElemento| Upper(oGet:VarGet()) == cElemento[1] } )
	If nPos == 0
		nPos := AAlist(aMatriz,{{nLin,nCol,nLin+4,nCol+18},;
							WhatColor(1),"Modo Faturamento",pSBAR})
