...
Function GetMedicao(aGetList,nLin,nCol,;      // menu em get
							lExib,nlinExib,nColExib,nLenExib)
	Local oGet := GetActive()
	Local aMatriz:={  {"Q","Quinzenal"},;
							{"M","Mensal   "},;
							{"N","Nao tem  "} }
	Local nPos
	nPos:=ascan(aMatriz,{ |cElemento| Upper(oGet:VarGet()) == cElemento[1] } )
	If nPos == 0
		nPos := AAlist(aMatriz,{{nLin,nCol,nLin+3,nCol+20},;
							WhatColor(1),"Contrato Medicao",pSBAR})
