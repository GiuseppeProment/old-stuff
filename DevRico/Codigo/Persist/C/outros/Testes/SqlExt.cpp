...
if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
   ...
   if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
      ...
      if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
         ...
         retcode = SQLConnect(hdbc, (SQLCHAR*) "interbase", SQL_NTS,
         ...
         if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO){
            ...
            /* Process data */
               ...
               retcode = SQLExecDirect(hstmt,/*(SQLCHAR*)*/ linha , SQL_NTS);
							 ...
							 cout << "-- rowCount deu = " << RowCount << endl;
               //retcode = SQLFetch( hstmt );
               showErroExec(hstmt,retcode);
               ExibaDados(hstmt);

							 cout << "2ø EXECU€ÇO õõõõõõõõõ" << endl;
...
void ExibaDados(SQLHSTMT &hstmt)
{
   ...
   for(SQLSMALLINT i=1;i<=Count;i++) {
	...
	cout << "*** retorno de sql_fetch_first =" << retcode << endl;
  showErroExec(hstmt,retcode);
